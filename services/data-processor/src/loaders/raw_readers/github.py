...
llama_index/readers/github/repository/base.py

The official implementation uses a nested event loop to fetch the contents of the repository.
Namely, loop.run_until_complete is used to "synchronously" wait for the async results. This collides
with FastAPI uvicorn's event loop. To circumvent this, we copy their code and use "await" instead.
"""
...
class GithubRepositoryReader(BaseReader):
    ...
    async def _load_data_from_commit(self, commit_sha: str) -> List[Document]:
        """
        ...
        """
        commit_response: GitCommitResponseModel = await self._github_client.get_commit(
            self._owner, self._repo, commit_sha, timeout=self._timeout
        )

        tree_sha = commit_response.commit.tree.sha
        blobs_and_paths = await self._recurse_tree(tree_sha)

        print_if_verbose(self._verbose, f"got {len(blobs_and_paths)} blobs")

        return await self._generate_documents(
            blobs_and_paths=blobs_and_paths, id=commit_sha
        )

    async def _load_data_from_branch(self, branch: str) -> List[Document]:
        """
        ...
        :return: list of documents
        """
        branch_data: GitBranchResponseModel = await self._github_client.get_branch(
        ...
        return await self._generate_documents(
            blobs_and_paths=blobs_and_paths, id=branch
        )
    ...
    async def load_data(
        ...
        if commit_sha is not None:
            return await self._load_data_from_commit(commit_sha)

        if branch is not None:
            return await self._load_data_from_branch(branch)
